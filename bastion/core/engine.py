...
class NftablesBackend:
    ...
    def __init__(self, demo_mode: bool = False) -> None:
        ...
        self._last_ruleset: str | None = None  # For rollback
    ...
    def apply_ruleset(self, ruleset: RuleSet) -> dict:
        ...
        try:
            if not self.demo_mode:
                self._delete_existing_tables(ruleset)
            self._exec_nft_script(script)
            ruleset.last_applied = datetime.now(timezone.utc)

            enabled_rules = [r for r in ruleset.get_all_rules() if r.state == RuleState.ENABLED]
    ...
    def _delete_existing_tables(self, ruleset: RuleSet) -> None:
        """Delete existing Bastion-managed tables before applying a replacement ruleset."""
        for table in ruleset.tables.values():
            self._exec_nft(
                "delete",
                "table",
    ...
    def get_current_ruleset_json(self) -> dict[str, Any]:
        """Get the current nftables ruleset as JSON."""
        if self.demo_mode:
            return {"nftables": []}
        result = self._exec_nft("-j", "list", "ruleset")
        return cast(dict[str, Any], json.loads(cast(str, result.stdout)))
