...
class FirewallRule:
    ...
    source_address: Optional[str] = None  # CIDR notation: 192.168.1.0/24
    destination_address: Optional[str] = None
    source_port: Optional[PortRange] = None
    destination_port: Optional[PortRange] = None
    interface_in: Optional[str] = None  # e.g., eth0
    interface_out: Optional[str] = None

    # Action
    action: Action = Action.DROP
    jump_target: Optional[str] = None  # chain name if action is JUMP
    rate_limit: Optional[RateLimit] = None  # config if action is RATE_LIMIT
    log_prefix: Optional[str] = None  # log prefix if action is LOG
        ...
        if "/" in cidr:
            ...
            try:
                ...
                if not 0 <= prefix_int <= 128:
                    errors.append(f"{field_name}: Invalid prefix length {prefix}")
            except ValueError:
                errors.append(f"{field_name}: Invalid prefix '{prefix}'")
        return errors
...
class Chain:
    ...
    hook: Optional[str] = None  # input, output, forward, prerouting, postrouting
    priority: int = 0
...
class RuleSet:
    ...
    def find_rule(self, rule_id: str) -> Optional[FirewallRule]:
